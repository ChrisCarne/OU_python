...
# Output list of isotopes studied, tweak to get punctuation right (yes I am that sad).
# There's probably a better way to do this but this works so...
def do_iso_string(isotopes):
    ...
    for n in range(len(isotopes)):
        ...
        elif n==len(isotopes)-2: #if the isotope is the second to last in the list
...
while True:
  ...
  plt.clf() #clears existing graph
  do_graph(time_series, isotope_name) #Creates a graph
